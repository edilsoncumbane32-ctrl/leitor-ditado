...
// ==================== LEITURA (sem pausas internas) ====================
async function lerFrase(idx) {
    ...
    statusSpan.innerText = `🔊 Lendo frase ${idx+1}...`;
    try {
        await SpeechController.speak(frase, velocidade);
        statusSpan.innerText = '✅ Concluído.';
        if (emModoContinuo && !modoDitado.checked && idx+1 < frases.length) {
            indiceAtual = idx+1;
            atualizarDisplayFrase();
...
function repetirFrase() { if(audioAtivo && frases.length) lerFrase(indiceAtual); }
function voltar() {
    if(!audioAtivo) return;
    if(indiceAtual > 0) { indiceAtual--; atualizarDisplayFrase(); lerFrase(indiceAtual); }
    else statusSpan.innerText = 'Primeira frase.';
}
function proximo() {
    if(!audioAtivo) return;
    if(indiceAtual+1 < frases.length) { indiceAtual++; atualizarDisplayFrase(); lerFrase(indiceAtual); }
...
function limparTexto() { texto.value = ''; atualizarFrases(); pararTudo(); salvarUltimoTexto(); statusSpan.innerText = 'Texto limpo.'; }

// ==================== COMPARTILHAR, IMPORTAR, OCR, TRADUÇÃO, VOZ ====================
...
function salvarVelocidade() { localStorage.setItem('velocidadeLeitor', velocidadeSlider.value); }
function salvarPausaEntreFrases() { pausaEntreFrases = parseFloat(pausaSlider.value); localStorage.setItem('pausaLeitor', pausaEntreFrases); pausaValor.innerText = pausaEntreFrases; }
function salvarModoDitado() { localStorage.setItem('modoDitadoLeitor', modoDitado.checked); }
function alternarTema() {
    document.body.classList.toggle('dark-mode');
    localStorage.setItem('temaLeitor', document.body.classList.contains('dark-mode') ? 'dark' : 'light');
    themeToggle.textContent = document.body.classList.contains('dark-mode') ? '☀️' : '🌙';
}
function carregarTema() {
    if (localStorage.getItem('temaLeitor') === 'dark') {
        document.body.classList.add('dark-mode');
        themeToggle.textContent = '☀️';
    } else themeToggle.textContent = '🌙';
}
async function ativarAudio() {
    statusSpan.innerText = '🎤 Ativando...';
    if (await SpeechController.activate()) {
        audioAtivo = true;
        audioActivationDiv.style.display = 'none';
        document.querySelectorAll('button, input, select').forEach(el => el.disabled = false);
        btnLer.disabled = false; btnRepetir.disabled = false; btnVoltar.disabled = false;
        btnProximo.disabled = false; btnParar.disabled = false; modoDitado.disabled = false;
        statusSpan.innerText = '✅ Áudio ativado!';
        carregarConfiguracoes();
    } else statusSpan.innerText = '❌ Falha. Tente novamente.';
}
function abrirModal() { configModal.style.display = 'flex'; }
function fecharModal() { configModal.style.display = 'none'; }
window.onclick = (e) => { if(e.target === configModal) fecharModal(); };
closeModal.addEventListener('click', fecharModal);
btnConfig.addEventListener('click', abrirModal);
btnSalvarConfig.addEventListener('click', salvarConfiguracoes);
themeToggle.addEventListener('click', alternarTema);
velocidadeSlider.addEventListener('input', (e)=>{ velValor.innerText=e.target.value; salvarVelocidade(); });
velMenos.addEventListener('click',()=>{ velocidadeSlider.stepDown(); velValor.innerText=velocidadeSlider.value; salvarVelocidade(); });
velMais.addEventListener('click',()=>{ velocidadeSlider.stepUp(); velValor.innerText=velocidadeSlider.value; salvarVelocidade(); });
pausaSlider.addEventListener('input', (e)=>{ pausaValor.innerText=e.target.value; salvarPausaEntreFrases(); });
modoDitado.addEventListener('change', salvarModoDitado);
fonteSelect.addEventListener('change', salvarConfiguracoes);
tamanhoSlider.addEventListener('input', (e)=>{ tamanhoValor.innerText=e.target.value+'px'; salvarConfiguracoes(); });
texto.addEventListener('input', ()=>{ atualizarFrases(); if(estaEsperando) pararTudo(); });
historySelect.addEventListener('change', (e)=>{ if(e.target.value!=='') carregarHistorico(parseInt(e.target.value)); historySelect.value=''; });
favoritosSelect.addEventListener('change', (e)=>{ if(e.target.value!=='') carregarFavorito(parseInt(e.target.value)); favoritosSelect.value=''; });
btnLer.addEventListener('click', lerTudo);
btnRepetir.addEventListener('click', repetirFrase);
...
btnImportar.addEventListener('click', importarTxt);
btnCompartilhar.addEventListener('click', compartilharTexto);
btnTraduzir.addEventListener('click', traduzirTexto);
btnFavorito.addEventListener('click', () => { if(favoritos.some(f=>f.texto===texto.value.trim())) removerFavorito(); else adicionarFavorito(); });
micBtn.addEventListener('click', startListening);
btnActivate.addEventListener('click', ativarAudio);
window.addEventListener('load', ()=>{
    carregarTema();
    if(!window.speechSynthesis) statusSpan.innerText='Sem suporte a voz.';
    else statusSpan.innerText='🔇 Ative o áudio';
    atualizarSelectHistorico();
    atualizarFavoritos();
    atualizarFrases();
});